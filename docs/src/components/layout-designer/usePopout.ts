...
export function usePopout(title: () => string, opts: PopoutOptions = {}) {
  ...
  // requestWindow() is async and pipTarget isn't set until it resolves, so a second open() fired
  // while one is still pending (e.g. a fast double-click) would pass the pipTarget guard and spawn
  // a duplicate window, orphaning the first. Gate on the in-flight request too.
  let opening = false

  async function open() {
    if (!supported || pipTarget.value || opening) return
    opening = true
