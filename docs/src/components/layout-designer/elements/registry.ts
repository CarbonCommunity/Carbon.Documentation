...
import type { CuiComponent, DesignerElement, ElementType } from '../types'
...
export const ELEMENT_DEFINITIONS: ElementDefinition[] = [
  widen(panelDefinition),
  widen(textDefinition),
  widen(buttonDefinition),
...
/**
 * AddUi wire-component types whose client widget is stateful (see `ElementDefinition.dynamicWireComponents`),
 * merged across every registered element. Derived so an element module can't drift from the live-preview
 * diff (previewDiff.ts) that decides whether to patch or destroy+recreate.
 */
export const DYNAMIC_WIRE_COMPONENT_TYPES: Set<CuiComponent['type']> = new Set(ELEMENT_DEFINITIONS.flatMap((d) => d.dynamicWireComponents ?? []))
