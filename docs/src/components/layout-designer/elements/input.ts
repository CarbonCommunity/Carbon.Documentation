...
function carbon(el: InputElement, ctx: EmitContext): string[] {
  ...
  return [
    ...
    `    ${posExpr(el)},`,
    `    ${offExpr(el)},`,
    `    "${color(t.color)}", "${esc(t.text)}", ${intStr(t.fontSize)}, "${esc(t.command)}", ${clampInt(t.charLimit)}, ${t.isProtected}, CUI.Handler.FontTypes.${fontDef(t.font).id}, TextAnchor.${t.align}, "${name}");`,
    '',
  ]
}
