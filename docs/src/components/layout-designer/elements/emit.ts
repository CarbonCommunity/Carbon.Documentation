...
export interface ElementDefinition<E extends DesignerElement = DesignerElement> {
  ...
  /**
   * AddUi wire-component `type` strings this element emits whose client widget is stateful — patching
   * one in place (an `update:true` re-send) re-initialises the widget rather than adjusting it, which
   * disturbs running state (e.g. a countdown timer jumps/restarts, an input field loses focus/text).
   * The live-preview diff (previewDiff.ts) uses this to leave an unchanged instance running untouched
   * and only destroy+recreate it when its own content changes. Omit for elements with no such component.
   */
  dynamicWireComponents?: CuiComponent['type'][]
}
