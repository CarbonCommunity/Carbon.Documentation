...
export function generateFullClass(elements: DesignerElement[], provider: Provider, rootLayer: ClientPanel = 'Overlay', dataSources: DataSource[] = []): string {
  ...
  // Buttons that capture a command get a matching handler stub after the builder method.
  const stubs = commandStubs(elements)
  const methods = stubs.length ? [...method, '', ...stubs] : method
  if (provider === 'both') {
    return [
      '#if CARBON',
