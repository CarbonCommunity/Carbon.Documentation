...
const GALLERY_GROUPS: { title: string; items: ExampleLayout[] }[] = (
  ...
  // all-examples is the consolidated tabbed layout - loadable via File > Load examples, but it IS
  // the other cards, so the gallery doesn't list it.
).map(([title, cat]) => ({ title, items: EXAMPLE_LAYOUTS.filter((e) => e.category === cat && e.id !== 'all-examples') }))
</script>
