...
// Capture phase: canvas/tree rows stopPropagation on pointerdown, so a bubble listener would miss clicks.
useEventListener(
  window,
  'pointerdown',
  (e: PointerEvent) => {
    if (e.button === 2) return // right-press re-opens the menu elsewhere; don't fight it
    if (!(e.target as HTMLElement)?.closest('.ld-ctxmenu')) emit('close')
  },
  true,
)
