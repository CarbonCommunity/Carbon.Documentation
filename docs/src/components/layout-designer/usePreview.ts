...
import { chunkAddUiJson, diffPreview } from './previewDiff'
...
function pushSnapshot() {
  ...
  // Chunked to stay under the server's per-string read cap — one oversized send is silently
  // dropped server-side, which reads as "preview just doesn't draw". An empty payload (all
  // untouched dynamics) sends nothing.
  const chunks = chunkAddUiJson(payload)
  for (const json of chunks) {
    sv.sendCall(RPC_ADD, pid, json)
  }
  if (chunks.length > 1) logInfo(`AddUi payload split into ${chunks.length} sends (size cap)`)
  if (chunks.length) logAddUi(payload)
...
function startPreview() {
  ...
  pushSnapshot()
  // Re-push (200ms trailing debounce) on any edit to elements or canvas (rootLayer/aspect). The name
  // diff handles add/move/remove and even a full layout switch, so no special-casing here.
  // vueuse exposes no cancel handle for a pending trailing fire, so the callback checks it still
  // belongs to the active watcher — a push scheduled just before stopPreview() must not land after
  // it (this replaces the old explicit clearTimeout on stop).
  const stop = watchDebounced(
    [elements, dataSources, () => ({ ...canvas })],
    () => {
      if (stopWatch === stop) pushSnapshot()
    },
    { debounce: DEBOUNCE_MS, deep: true }
  )
  stopWatch = stop
}
