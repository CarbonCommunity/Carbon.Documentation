...
<template>
  <div v-if="selectedServer?.hasPermission('profiler_edit')">
    <div class="flex pb-5 text-sm gap-x-1 items-center" v-if="selectedServer.ProfileState.IsEnabled && !selectedServer.ProfileState.HasCrashed">
      <button v-if="!selectedServer.ProfileState.IsProfiling" class="r-send-button !p-3 !text-green-400 !bg-green-800/20 hover:!bg-green-800/80 hover:!text-green-200" @click="toggleProfile(false)"><Play :size=18 /></button>
      <button v-if="selectedServer.ProfileState.IsProfiling" class="r-send-button !p-3 !text-slate-400 !bg-slate-500/20 hover:!bg-slate-500/80 hover:!text-slate-200" @click="toggleProfile(false)"><Square :size=18 /></button>
      <button v-if="selectedServer.ProfileState.IsProfiling" class="r-send-button !p-3 !text-red-400 !bg-red-800/20 hover:!bg-red-800/80 hover:!text-red-200 uppercase" @click="toggleProfile(true)"><X :size=18 /></button>
