...
import ServerMapImage from '../common/ServerMapImage.vue'
import { selectedServer } from './ControlPanel.SaveLoad'

onMounted(() => {
  if(selectedServer.value?.MapInfo == null) {
    selectedServer.value?.sendCall('LoadMapInfo')
  }
  selectedServer.value?.startMapEntityTracking()
})
onUnmounted(() => {
