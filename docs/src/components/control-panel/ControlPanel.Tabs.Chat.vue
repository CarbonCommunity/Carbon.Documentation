...
<template>
  ...
  <div v-if="selectedServer?.hasPermission('chat_input')" class="flex gap-2" style="align-items: center; background-color: var(--vp-code-copy-code-bg); padding: 10px">
    <div :class="`max-w-${showingChatColorPicker ? '64' : '16'}`">
      <input class="w-full text-right" :disabled="!selectedServer.Bridge" :style="{ color: selectedServer.Bridge ? selectedServer.ChatColor : '#fff' }" @keyup.enter="save()" @click="if(selectedServer.Bridge) { showingChatColorPicker = !showingChatColorPicker; save() }" style="font-family: monospace;" spellcheck="false" v-model="selectedServer.ChatUsername"/>
